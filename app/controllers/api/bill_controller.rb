class Api::BillController < Api::ApplicationController
  before_action :load_bill, except: [:index, :create]
  ...
  def bill_as_json(bill)
    bill.as_json(only: [:id, :name, :amount_cents, :payed, :payment_day, :repetition_type, :created_at, :account_id, :updated_at])
  end
